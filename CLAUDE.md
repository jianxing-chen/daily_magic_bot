...
2. **News Fetching** ([news_fetcher.py](news_fetcher.py)) — `MultiSourceNewsFetcher` with 13 sources across 4 groups: Nature web, Nature RSS (4 feeds), Science RSS, ScienceDaily RSS (4 feeds), Psychology RSS (3 feeds: PsyPost, Neuroscience News, PNAS). Parallel fetch via `ThreadPoolExecutor(max_workers=8)`, then dedup by URL, then filter to last 1 day.
