...
class WeatherParser:
    ...
    def get_weather_forecast(self) -> Dict:
        ...
        try:
            ...
            result['city'] = crumbs[1].text.strip() if len(crumbs) >= 2 else '未知'

            # 获取当前实况温度 (从实况区域)
            current_temp_text = self._safe_extract_text('div.sk div.tem span', '')
            result['current_temp'] = current_temp_text + '°C' if current_temp_text else '未知'

            # 获取白天天气状况
            result['weather'] = self._safe_extract_text('div.t ul li p.wea')
            ...
            if wind_elem:
                ...
                if wind_dir:
                    result['wind'] = f"{wind_dir} {wind_level}"
                else:
                    # 兜底：从 title 属性获取
                    wind_title = wind_elem.get('title', '')
                    result['wind'] = wind_title if wind_title else wind_level
            else:
                result['wind'] = '未知'
